...
from sab import falpha, SABC_ATE
...
typ_list = [3, 2] # phi function type list
nk_list = [(100, 5), (10, 50)] #
...
for (typ, (n, k), delta) in product(typ_list, nk_list, delta_list):
    ...
    name = 'result/ATE/'+'typ'+str(typ)+'n'+str(n)+'k'+str(k)+'delta'+str(delta)+'_nonlinear_adaptive_ATE.npz'
    ...
    def one_step(seed):
        ...
        sabcs = [SABC_ATE(alpha=alpha, B=B, p=p) for _ in range(lm)]
        ...
        for j in range(k):
            if j == 0:
                ...
                y1 = 1 + (X1[:,0]-X1[:,1])/2 + phi((X1[:,0]+X1[:,1])/sqrt(2.), delta, typ)*(X1[:,2])**2 + \
                            np.random.randn(this_n)*e
                X0 = sm.add_constant(X0)
                X1 = sm.add_constant(X1)
            else:
                this_n = n
                X = np.random.randn(2*this_n, nfeatures).dot(L).clip(lb, ub)
                ### adaptive randomization
                X_ = sm.add_constant(X)
                model_t = sabcs[0] # anyone should work
                beta0_hat, beta1_hat = model_t.beta0, model_t.beta1
                inds = X_.dot(beta1_hat-beta0_hat)
                ...
                y0, y1 = None, None
                if len(X0) > 0:
                    y0 = 1 + (X0[:,0]-X0[:,1])/2+ np.random.randn(X0.shape[0])*e
                    X0 = sm.add_constant(X0)
                if len(X1) > 0:
                    y1 = 1 + (X1[:,0]-X1[:,1])/2 + phi((X1[:,0]+X1[:,1])/sqrt(2.), delta, typ)*(X1[:,2])**2 + \
                                np.random.randn(X1.shape[0])*e
                    X1 = sm.add_constant(X1)
